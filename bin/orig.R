...
install.packages('egg', dependencies = TRUE)  # for ggarrange
install.packages('gridExtra')
install.packages('ggthemes')
install.packages('ggcorrplot')  # correlations
install.packages('GGally')
install.packages('psych')   # to use with describe function
...
devtools::install_github("ricardo-bion/ggradar", 
                         dependencies = TRUE)
library(sjPlot)
library(psych)              # to use with describe function
library(ggplot2)
library(plotly)
library(dplyr)          ## load
...
library(gridExtra)
library(grid)
library(egg) #combined ggplots
library(ggpubr) # for annotate_figure
library(lattice) # impute density plot
library(stringr)
library(mice)
library(corrplot)
library(caret)  
library(Rtsne)
require(biotools)  # mahalanobis dist
library(mclust)   # GMM
...
ggsave(plot=out, width=6, height=4, dpi=300, filename ='outliers.pdf')
dev.off()